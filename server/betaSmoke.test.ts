...
import { LocalScratchRandomnessProvider } from './randomness.js';
...
test('canonical closed beta product and trust loop', async () => {
  ...
  const economy = new EconomyService(economyRepository, new DevelopmentPaymentVerifier(config), new LocalScratchRandomnessProvider(), config);
  ...
  await draws.closeDraw('beta-smoke-global', '2026-01-02T00:00:00.000Z'); await draws.requestRandomness('beta-smoke-global'); await draws.resolveDraw('beta-smoke-global');
  ...
  assert(persisted.ledger.every((entry) => entry.classification !== 'simulated_scratch_prize' || !entry.spendable));
});
