...
function signedInteger(value: unknown, name: string): bigint { const result = text(value, name); if (!/^-?(0|[1-9][0-9]*)$/.test(result)) throw new Error(`${name}_invalid`); return BigInt(result); }
...
function parseFounder(value: unknown): FounderControlMetrics {
  const row = record(value, 'founder_metrics'); const human = record(row.humanClaim, 'human_claim'); const product = record(row.product, 'product'); const genesis = record(row.genesis, 'genesis'); const trust = record(row.trust, 'trust'); const operations = record(row.operations, 'operations');
  return {
    generatedAt: text(row.generatedAt, 'generated_at'),
    humanClaim: { period: nullableText(human.period, 'period'), poolUnits: integer(human.poolUnits, 'pool_units'), participants: integer(human.participants, 'participants'), settledUnits: integer(human.settledUnits, 'settled_units'), settledUnitsPerHuman: integer(human.settledUnitsPerHuman, 'settled_units_per_human'), unissuedUnits: integer(human.unissuedUnits, 'unissued_units'), previousPeriodParticipants: integer(human.previousPeriodParticipants, 'previous_participants'), participantGrowthBps: human.participantGrowthBps === null ? null : signedInteger(human.participantGrowthBps, 'growth_bps'), projectedShare2x: integer(human.projectedShare2x, 'projected_share_2x'), projectedShare5x: integer(human.projectedShare5x, 'projected_share_5x'), projectedShare10x: integer(human.projectedShare10x, 'projected_share_10x') },
    product: { users: integer(product.users, 'users'), verifiedHumans: integer(product.verifiedHumans, 'verified_humans'), titlesIssued: integer(product.titlesIssued, 'titles_issued'), settledPurchases: integer(product.settledPurchases, 'settled_purchases'), activeCampaignId: nullableText(product.activeCampaignId, 'active_campaign_id'), monthlyDrawStatus: nullableText(product.monthlyDrawStatus, 'monthly_draw_status'), quarterlyDrawStatus: nullableText(product.quarterlyDrawStatus, 'quarterly_draw_status') },
    genesis: { campaignId: nullableText(genesis.campaignId, 'campaign_id'), budgetUnits: integer(genesis.budgetUnits, 'budget_units'), distributedUnits: integer(genesis.distributedUnits, 'distributed_units'), reservedUnits: integer(genesis.reservedUnits, 'reserved_units'), remainingUnits: integer(genesis.remainingUnits, 'remaining_units'), participants: integer(genesis.participants, 'participants'), byQuest: array(genesis.byQuest, 'by_quest').map((item) => { const entry = record(item, 'quest_metric'); return { questId: text(entry.questId, 'quest_id'), qualified: integer(entry.qualified, 'qualified'), claimed: integer(entry.claimed, 'claimed'), distributedUnits: integer(entry.distributedUnits, 'distributed_units') }; }), verifiedReferrals: integer(genesis.verifiedReferrals, 'verified_referrals'), milestoneQualifications: integer(genesis.milestoneQualifications, 'milestones'), externalPending: integer(genesis.externalPending, 'external_pending') },
    cap: parseTotals(row.cap),
    trust: { immutableLedgerRows: integer(trust.immutableLedgerRows, 'ledger_rows'), accountingMode: 'simulated', productionTokenTransfers: false, latestDrawId: nullableText(trust.latestDrawId, 'latest_draw_id'), manifestCommitment: nullableText(trust.manifestCommitment, 'manifest_commitment'), randomnessStatus: text(trust.randomnessStatus, 'randomness_status'), externalProofStatus: text(trust.externalProofStatus, 'external_proof_status'), anchorStatus: text(trust.anchorStatus, 'anchor_status'), verifyDrawStatus: text(trust.verifyDrawStatus, 'verify_draw_status') },
    operations: { reconciliationPending: integer(operations.reconciliationPending, 'reconciliation_pending'), reconciliationFailedOrStuck: integer(operations.reconciliationFailedOrStuck, 'reconciliation_failed'), drawJobsFailed: integer(operations.drawJobsFailed, 'draw_jobs_failed'), readinessStatus: text(operations.readinessStatus, 'readiness_status') },
  };
}
