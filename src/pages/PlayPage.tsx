...
export function PlayPage({ onNavigate }: { onNavigate: (tab: Tab) => void; notify: (message: string) => void }) {
  ...
  if (lastReveal && outcomeTitle) {
    ...
    return <div className="result-wrap"><section className={`prize-result ${won ? 'winner' : ''}`}><div className="result-rays" aria-hidden="true" /><span className="result-icon">{won ? <Trophy /> : <Sparkles />}</span><p className="eyebrow">Persisted instant result</p><h1>{won ? formatWld(lastReveal.result.prizeUnits) : 'Not this time'}</h1><p>{won ? 'This is a simulated prize liability—not spendable WLD and not an on-chain payout.' : 'This title remains live for the monthly draw and annual jackpot.'}</p><div className="result-title"><Ticket size={17} /><span>{outcomeTitle.serial}</span><b><Check size={15} /> Draw eligible</b></div>{won ? <button className="primary-button wide" onClick={() => onNavigate('wallet')}>View simulated liability <ArrowRight size={17} /></button> : null}<button className="text-button centered" onClick={clearLastReveal}>{unrevealed.length ? 'Scratch another title' : 'Close result'}</button></section></div>;
  }
