...
export function SocialPage({ notify }: { notify: (message: string) => void }) {
  ...
  const submit = async () => { if (!await createPost(body)) { notify('Post must be valid and the social service must be available.'); return; } setBody(''); notify('Persisted to the CAP human feed.'); };
