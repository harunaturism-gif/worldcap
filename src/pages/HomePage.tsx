...
export function HomePage({ onNavigate, notify }: { onNavigate: (tab: Tab) => void; notify: (message: string) => void }) {
  ...
  return <div className="page-stack">
    <section className="hero-card">
      ...
      <div className="jackpot-lockup"><span className="jackpot-icon"><Trophy /></span><div><small>Set aside for the jackpot</small><h1>{formatWld(snapshot.pools.quarterly_jackpot)}</h1></div></div>
      <p>Claiming CAP never removes your title from the quarterly jackpot.</p>
      ...
      <button className="fairness-cta" onClick={() => setFairnessOpen(true)}><ShieldCheck size={17} /> See exactly how winners are chosen</button>
      <div className="hero-fine"><span><CalendarDays size={14} /> Monthly draw {new Date(campaign.monthlyDrawAt).toLocaleDateString([], { month: 'short', day: 'numeric' })}</span><button onClick={() => onNavigate('play')}><Sparkles size={14} /> Open your free monthly claim <ChevronRight size={14} /></button></div>
    </section>

    <section className="stat-grid" aria-label="Campaign summary">
      <article><span className="stat-icon peach"><Ticket /></span><small>Titles issued</small><strong>{snapshot.titlesSold.toLocaleString()}</strong><em>Issued and saved</em></article>
      <article><span className="stat-icon mint"><CircleDollarSign /></span><small>This month's prize pool</small><strong>{formatWld(snapshot.pools.monthly_prize_pool)}</strong><em>38% of title purchases</em></article>
      <article><span className="stat-icon lilac"><Sparkles /></span><small>Your titles</small><strong>{myTitles.length}</strong><em>Still draw eligible</em></article>
    </section>
    ...
    <section className="panel allocation-panel"><div className="section-heading"><div><p className="eyebrow">Every verified purchase</p><h2>Where every WLD goes</h2></div><span className="simulation-badge">Open ledger</span></div><div className="allocation-bar" aria-label="40% CAP redemption, 38% monthly, 10% quarterly, 10% company, 2% platform"><i className="cap" /><i className="monthly" /><i className="jackpot" /><i className="company" /><i className="platform" /></div><div className="allocation-legend"><span><i className="cap" /><b>40%</b> CAP redemption</span><span><i className="monthly" /><b>38%</b> Monthly prizes</span><span><i className="jackpot" /><b>10%</b> Quarterly jackpot</span><span><i className="company" /><b>10%</b> Company treasury</span><span><i className="platform" /><b>2%</b> Platform & ops</span></div></section>
