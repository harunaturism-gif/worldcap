...
export function WalletPage() {
  ...
  const simulatedLiability = snapshot.ledger.filter((entry) => entry.classification === 'simulated_scratch_prize' || entry.classification === 'simulated_draw_prize').reduce((sum, entry) => sum + BigInt(entry.amountUnits), 0n);
  ...
  return <div className="page-stack">
    ...
    <section className="wallet-stat-grid"><article><span><CircleDollarSign /></span><small>Verified WLD spent</small><strong>{formatWld(verifiedSpend)}</strong></article><article><span><ShieldAlert /></span><small>Simulated liabilities</small><strong>{formatWld(simulatedLiability)}</strong></article><article><span><Landmark /></span><small>Quarterly allocation</small><strong>{formatWld(snapshot.pools.quarterly_jackpot)}</strong></article></section>
