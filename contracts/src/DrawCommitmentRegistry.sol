...
contract DrawCommitmentRegistry {
    ...
    function anchorDraw(bytes32 drawIdHash, bytes32 manifestRoot, uint256 eligibleCount, bytes32 algorithmVersionHash)
        external
    {
        if (msg.sender != anchorAuthority) revert Unauthorized();
        if (
            drawIdHash == bytes32(0) || manifestRoot == bytes32(0) || eligibleCount == 0
                || algorithmVersionHash == bytes32(0)
        ) {
