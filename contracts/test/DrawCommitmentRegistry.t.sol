...
contract UnauthorizedCaller {
    function anchor(DrawCommitmentRegistry registry, bytes32 drawId, bytes32 root, bytes32 algorithm)
        external
        returns (bool)
    {
...
contract DrawCommitmentRegistryTest {
    ...
    function setUp() public {
        registry = new DrawCommitmentRegistry(address(this));
    }
    ...
    function testAnchorsExactlyOnce() public {
        ...
        require(
            stored.manifestRoot == ROOT && stored.eligibleCount == 42 && stored.algorithmVersionHash == ALGORITHM,
            "bad anchor"
        );
        (bool overwritten,) =
            address(registry).call(abi.encodeCall(registry.anchorDraw, (DRAW, keccak256("replacement"), 99, ALGORITHM)));
    ...
    function testRejectsZeroAuthority() public {
        try new DrawCommitmentRegistry(address(0)) {
            revert("zero authority accepted");
        } catch {}
    }

    function testHasNoWinnerOrCustodyInput() public pure {
        require(
            DrawCommitmentRegistry.anchorDraw.selector
                == bytes4(keccak256("anchorDraw(bytes32,bytes32,uint256,bytes32)")),
            "unexpected ABI"
        );
    }
}
